...
export const ViewAllRoom = createAsyncThunk(
    ...
    async (thunkAPI) => {
        try {
            const response = await fetch(`http://localhost:3500/room`);
            if (!response.ok) {
...
export const ViewRoomCategoryById = createAsyncThunk(
    ...
    async (categoryRoomId, thunkAPI) => {
        try {
            const response = await fetch(`http://localhost:3500/category/${categoryRoomId}`);
            if (!response.ok) {
                throw new Error('Failed to fetch Room!')
            }
            const data = await response.json();
...
export const CreateRoom = createAsyncThunk(
    ...
    async ({ categoryRoomId }, thunkAPI) => {
        try {
            const response = await fetch(`http://localhost:3500/category/${categoryRoomId}/room`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
